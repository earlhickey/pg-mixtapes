  ...
  if ser.readable():
    ...
    if response :
      ...
      # gasUsage
        ...
        if (float(data[1:9]) - lastGasUsage) < 1 :
          dataList[6] = float(data[1:9])

    # test if all data is collected
    if data[0:1] == "!" and None not in dataList :
      break;
